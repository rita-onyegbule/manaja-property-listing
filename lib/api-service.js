...
/**
 * Get metro manager details for a property
 */
export async function getPropertyMetroManager(propertyId) {
  try {
    return await apiRequest(`/metro/properties/${propertyId}/metro-manager`);
  } catch (error) {
    console.error('Failed to fetch metro manager details:', error);
    return null;
  }
}
